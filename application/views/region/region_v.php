...
<div class="modal fade" role="dialog" id="frm_category">
            ...
            <div class="modal-content">
              ...
              <div class="modal-body" style="padding-bottom:2px;">
				...
				<div class="form-group">
                  <label>Region</label><span class="label label-danger pull-right region_error"></span>
                  <input type="text" class="form-control" name="region" id="region" value="" placeholder="Region" autocomplete="off" />
 ...
 <div class="box-header">
    <button class="btn btn-success add_category"><i class="fa fa-plus"></i> Add Region</button>
...
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			...
			<th style="text-align:center; width:76%">Region</th>			
			...
			<?php 
				...
				if(!empty($region)){		
					foreach($region as $r){	
						...
						$info = $r['id_region'].'Þ'.$r['region_name'];
						echo '<tr>';
						echo '<td align="center">'.$i++.'.</td>';						
						echo '<td>'.$r['region_name'].'</td>';	
						
						echo '<td align="center" style="vertical-align: middle;">		
			<a href="'.$view_sub.'"><button title="List City" class="btn btn-xs btn-warning"><i class="fa fa-list-ul"></i> List City</button></a>
			<a href="#" id="'.$info.'" title="Edit" class="edit_category"><button class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</button></a>
			<button title="Delete" id="'.$r['id_region'].'" class="btn btn-xs btn-danger del_category"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
						echo '</tr>';
					}
				}
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('region/del');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	...
	$('.region_error').text('');
	if(region <= 0 || region == '') {
		$('.category_error').text('Region harus diisi');
		return false;
	}	
	var dt = $('#frm_cat').serialize();
	var url = '<?php echo site_url('region/simpan');?>';
	$.ajax({
		data:dt,
		type:'POST',
		url : url,
