...
class Trans_coin extends MY_Controller {
	...
	public function index() {
		...
		$where = array();
		$where = array('transaksi.status'=>1,'transaksi.type'=>1);		
		$from = '';
		$to = '';
		if(!empty($tgl)){
	...
	public function confirm() {
		...
		$selects = array('transaksi.*','customer.nama','customer.last_name','customer.phone');
		$where = array();
		$where = array('transaksi.status'=>2,'transaksi.type'=>1);
		$from = '';
		$to = '';
		if(!empty($tgl)){
			$_tgl = !empty($tgl) ? explode('-', $tgl) : '';
	...
	public function appr() {
		...
		$where = array();
		$this->data['status'] = 4;
		$where = array('transaksi.status'=>4,'transaksi.type'=>1);
		$from = '';
		$to = '';
		if(!empty($tgl)){
			$_tgl = !empty($tgl) ? explode('-', $tgl) : '';
	...
	public function appr_reject(){
		...
		if((int)$_POST['status'] == 4){			
			...
			//$this->htg_trans_coin($id_member);
		}	
		if((int)$_POST['status'] == 3){
			$pesan = 'Pembelian Coin #'.$_POST['id_trans'].' tidak disetujui';
	...
	function htg_trans_coin($id_member=0){
		$tgl = date('Y-m-d H:i:s');
		$where = array();
		$where = array('transaksi.status'=>4,'transaksi.type'=>1,'id_member'=>$id_member);
		...
		if($cnt_transaksi == 5){
			$achievement = 1;
			$next_cnt_transaksi = 10;
		}
		if($cnt_transaksi > 9 && $cnt_transaksi % 10 == 0){
			$achievement = 1;
			$next_cnt_transaksi = $cnt_transaksi + 10;
		}
		$dt_achievement = array();
		if((int)$achievement > 0){
			$this->access->updatetable('achievement',array('status'=>0),array('id_member'=> (int)$id_member,'type' => 3,'status' =>-1));
			$cnt_achievement = $this->access->readtable('achievement',array('id_member'),array('id_member'=> (int)$id_member,'type' => 3,'status' =>-1))->row();
			...
			if($cnt_achievement > 0){
				...
				$dt_achievement = array(
					...
					'type'			=> 3,
					'title'			=> 'Achievement 3',
					'keterangan'	=> 'Bonus achievement transaksi coin '.$next_cnt_transaksi.'x',
					'coin'			=> 1,
					'status'		=> -2,
					'created_at'	=> $tgl
				);
	...
	public function export_r(){
		...
		$selects = array('transaksi.*','customer.nama','customer.last_name','customer.phone');
		$where = array();
		$where = array('transaksi.status'=>$status,'transaksi.type'=>1);
		$from = '';
		$to = '';
		...
		$filename = 'Transaksi Coin_'.$status_name;
		$this->excel->setActiveSheetIndex(0);
		$this->excel->getActiveSheet()->setTitle('data_transaksi');		
		$this->excel->getActiveSheet()->setCellValue('A1', 'Transaksi Coin - '.$status_name);		
        ...
        $this->excel->getActiveSheet()->setCellValue('E3', 'Jumlah Coin');
